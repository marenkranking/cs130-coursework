...
const getTracks = (term) => {
    document.querySelector('#tracks').innerHTML = `<div></div>`;
        ...
        .then(tracks => {
            if (tracks.length === 0) {
                ...
                for (track of fiveTracks) {
                    document.querySelector('#tracks').innerHTML += `<section class="track-item preview" data-preview-track="${track.preview_url}">
...
const getArtist = (term) => {
    document.querySelector('#artist').innerHTML = `<div></div>`;
    const search = document.querySelector('#search').value;
